...
Deno.test("when - the accessor says what a shorthand cannot", () => {
  ...
  assertEquals(holds(c, ctx({ issue: issueContext({ impact: Impact.Major }) })), true);
  assertEquals(holds(c, ctx({ issue: issueContext({ impact: Impact.Trivial }) })), true);
  assertEquals(holds(c, ctx({ issue: issueContext({ impact: Impact.Minor }) })), false);
});
...
Deno.test("when - a condition about something absent is false", () => {
  ...
  const noIssue = ctx({ file: { path: "src/a.ts", extension: ".ts", grammarId: "typescript" } });
...
Deno.test("when - a catalog with no entry leaves impact absent, and that is false", () => {
  const unknown = ctx({ issue: issueContext({ impact: undefined, category: undefined }) });
...
Deno.test("when - in() matches the file being examined", () => {
  const c = when.in("src/**");
  const inside = ctx({ file: { path: "src/deep/a.ts", extension: ".ts", grammarId: "" } });
  const outside = ctx({ file: { path: "tests/a.ts", extension: ".ts", grammarId: "" } });
...
Deno.test("when - kind() matches the name or the full kind", () => {
  ...
  assertEquals(holds(when.kind("similar-functions/duplicate-function"), i), true);
...
Deno.test("when - grammar() reads the file, not the issue", () => {
  const f = ctx({ file: { path: "a.ts", extension: ".ts", grammarId: "typescript" } });
...
Deno.test("when - composition", () => {
  ...
  assertEquals(holds(when.all(when.in("src/**"), when.impact.atLeast(Impact.Major)), i), true);
  assertEquals(holds(when.all(when.in("tests/**"), when.impact.atLeast(Impact.Major)), i), false);
  assertEquals(holds(when.any(when.in("tests/**"), when.impact.atLeast(Impact.Major)), i), true);
  assertEquals(holds(when.not(when.in("tests/**")), i), true);
  assertEquals(holds(when.in("src/**").and(when.confidence.atLeast(80)), i), true);
  assertEquals(holds(when.in("tests/**").or(when.confidence.atLeast(80)), i), true);
});
